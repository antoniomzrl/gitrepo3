...
//
G4_DECLARE_PHYSCONSTR_FACTORY(PenelopeLEPTSPhysics);


PenelopeLEPTSPhysics::PenelopeLEPTSPhysics(G4double highELimit, G4int ver)
  : G4VPhysicsConstructor("PenelopeLEPTSPhysics"), verbose(ver)
{
...
PenelopeLEPTSPhysics::PenelopeLEPTSPhysics(G4double highELimit, G4int ver, const G4String&)
  : G4VPhysicsConstructor("PenelopeLEPTSPhysics"), verbose(ver)
{
  G4LossTableManager::Instance();
  SetPhysicsType(bElectromagnetic);
...
PenelopeLEPTSPhysics::~PenelopeLEPTSPhysics()
{}


void PenelopeLEPTSPhysics::ConstructParticle()
{
// gamma
  G4Gamma::Gamma();
...
void PenelopeLEPTSPhysics::ConstructProcess()
{
  if(verbose > 1) {
