...
StandardAndDNAPhysics1::StandardAndDNAPhysics1():  G4VUserPhysicsList()
{
...
StandardAndDNAPhysics1::~StandardAndDNAPhysics1()
{}


void StandardAndDNAPhysics1::ConstructParticle()
{
  ConstructBosons();
  ConstructLeptons();
  ConstructBarions();
...
void StandardAndDNAPhysics1::ConstructBosons()
{ 
  // gamma
...
void StandardAndDNAPhysics1::ConstructLeptons()
{
  // leptons
...
void StandardAndDNAPhysics1::ConstructBarions()
{
  //  baryons
...
void StandardAndDNAPhysics1::ConstructProcess()
{
  AddTransportation();
  ConstructEM();
  ConstructGeneral();
...
void StandardAndDNAPhysics1::ConstructEM()
{
  ...
  em_config->SetExtraEmModel("proton","hIoni",
                             mod,"world",0.0,2*MeV, new G4IonFluctuations());

  mod = new G4BetheBlochIonGasModel();
  mod->SetActivationLowEnergyLimit(standEnergyLimit);
  em_config->SetExtraEmModel("proton","hIoni",
                             mod,"world",2*MeV,100*TeV,
                             new G4UniversalFluctuation());
  ...
  em_config->SetExtraEmModel("alpha","ionIoni",
                             mod,"world",0.0,2*MeV/massFactor,
                             new G4IonFluctuations());

  mod = new G4BetheBlochIonGasModel();
  mod->SetActivationLowEnergyLimit(standEnergyLimit);
  em_config->SetExtraEmModel("alpha","ionIoni",
                             mod,"world",2*MeV/massFactor,100*TeV,
                             new G4UniversalFluctuation());
  ...
  em_config->SetExtraEmModel("alpha","alpha_G4DNAIonisation",
                             mod,"world",0.0,10*MeV);

  mod = new G4DNAMillerGreenExcitationModel();
  em_config->SetExtraEmModel("alpha","alpha_G4DNAExcitation",
                             mod,"world",1*keV,10*MeV);

}


void StandardAndDNAPhysics1::ConstructGeneral()
{ }


void StandardAndDNAPhysics1::SetCuts()
{
  if (verboseLevel >0)
  {
    G4cout << "StandardAndDNAPhysics1::SetCuts:";
