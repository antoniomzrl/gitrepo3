...
CHB=":MIXT_BY_NATOMS PIRIDINA $Dens 3  C 5 H 5 N 1
     :VOLU chb TUBE 0.0 8*cm/2 4*cm/2 PIRIDINA
...
GENDIST="$GENMONO	
	 /gamos/generator/energyDist gn GmGenerDistEnergyFromFile e15eVpiridina.txt interpolate 1*eV"
...
Resultados de las simulaciones con piridina.
...
(2) Modelo utilizado:
   ...
   Modelo SMF (strong mag field) consistente en suponer que el campo es infinito y eliminar la energía
   transversal tras el choque en el primario y al generar secundarios. Esto me obligó en tiempos a escribir
   una física modificada para generar la aproximación.
