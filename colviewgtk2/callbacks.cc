...
void CB_SaveImage() {
  ...
  SaveImage(0, 0, glw->allocation.width, glw->allocation.height);
}
...
void CB_PlotType(GtkComboBox * combo) {
  string plty = gtk_combo_box_get_active_text(combo);


  if(     plty == "Particle")    d->vType = 0;
  else if(plty == "Interaction") d->vType = 1;
  else if(plty == "Energy")      d->vType = 3;
  else if(plty == "ELost")       d->vType = 4;
  else if(plty == "EDep")        d->vType = 5;

  if(plty == "ColorBar") d->vColorBar = 1;
  else                   d->vColorBar = 0;

  cout << "CB_PlotType " << plty << " " << d->vType << endl;
  d->NeedUpdatePV = true;
  Invalidate(glw);
}
...
void CB_FontSelection(GtkWidget *widget, gpointer label) {
  ...
  if(result == GTK_RESPONSE_OK || result == GTK_RESPONSE_APPLY) {
    ...
    gtk_widget_modify_font(GTK_WIDGET(butwin), font_desc);
    g_free(fontname);
...
string ChooseFile(string path, string pattern) {
  ...
  GtkWidget * dialog =
    gtk_file_chooser_dialog_new( title.c_str(), (GtkWindow*)butwin,
...
// void CB_OpenDataFile(void) {
//   GtkWidget * dialog =
//     gtk_file_chooser_dialog_new( "Select Data File", (GtkWindow*)butwin, GTK_FILE_CHOOSER_ACTION_OPEN,
...
char * ChooseFileFromList(char * filter) {
  ...
  GtkWidget * fileWg = gtk_file_chooser_dialog_new("Select Paramet. File", (GtkWindow*)butwin,
